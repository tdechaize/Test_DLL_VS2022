...
#   mydll._Hello(None)
    ...
    mydll.Hello(None)
    print(f"----------------------       Lancement des operations arithmetiques avec des entiers        -----------------------");
    mydll.Addint.argtypes = [ctypes.c_short, ctypes.c_short]
    mydll.Addint.restype = ctypes.c_short
    result = mydll.Addint(42,7)
    print(f"La somme de 42 plus 7 vaut {result}.                  (from script python {sys.argv[0]})")
    mydll.Subint.argtypes = [ctypes.c_long, ctypes.c_long]
    mydll.Subint.restype = ctypes.c_long
    ...
    mydll.Divint.argtypes = [ctypes.c_long, ctypes.c_long]
    mydll.Divint.restype = ctypes.c_long
